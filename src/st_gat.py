...
class STGAT(nn.Module):
    """
        ...
        - out_channels: number of output channels of the GAT first layer, i.e. number of total output features per node; normally out_channels := in_channels
    """
    def __init__(self, in_channels, out_channels, N_nodes, gat_heads = 8, dropout = 0., lstm1_hidden_size = 32, lstm2_hidden_size = 128, **kwargs):
        print('Ignored kwargs in model:', kwargs)
        super(STGAT, self).__init__()

        self.dropout = nn.Dropout(p = dropout)
        self.gat = geomnn.GATConv(in_channels= in_channels, out_channels= out_channels, heads = gat_heads, dropout = 0, concat = False)

        self.N_preds = 9 # number of timepoints to predict
        ...
        self.linear = nn.Linear(lstm2_hidden_size, N_nodes * self.N_preds)
    ...
    def forward(self, batch, device):
        ...
        out = out.reshape(batch_size * N_nodes, self.N_preds)
